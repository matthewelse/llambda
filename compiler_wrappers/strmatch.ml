...
module Make (I : I) = struct
  ...
  let pat_as_string p =
    let rec digits k n p =
      ...
      then k
        ...
        digits (d :: k) (n - 1) (Nativeint.shift_right_logical p 8))
    in
    let ds = digits [] Arch.size_addr p in
    let ds = if Arch.big_endian then ds else List.rev ds in
    String.concat "" ds
  ;;
  ...
  let pp_match chan tag idxs cases =
    Printf.eprintf "%s: idx=[%s]\n" tag (String.concat "; " (List.map Int.to_string idxs));
    do_pp_cases chan cases
  ;;
  ...
  let mk_let_cell id str ind body =
    ...
    let cell =
      Cop
        ( Cload (Word_int, Asttypes.Mutable)
  ...
  let mk_cmp_gen cmp_op id nat ifso ifnot =
    let dbg = Debuginfo.none in
    let test = Cop (Ccmpi cmp_op, [ Cvar id; Cconst_natint (nat, dbg) ], dbg) in
  ...
  let interval m0 n =
    let rec do_rec m = if m >= n then [] else m :: do_rec (m + 1) in
    do_rec m0
  ;;
  ...
  let pat_of_string str =
    ...
    let get_byte i =
      if i < len
      then int_of_char str.[i]
      else if i < (n * Arch.size_addr) - 1
      then 0
      else (n * Arch.size_addr) - 1 - len
    in
    let mk_word ind =
      let w = ref 0n in
      ...
      then
        for i = imin to imax do
          w := Nativeint.logor (Nativeint.shift_left !w 8) (Nativeint.of_int (get_byte i))
        done
      else
      ...
      !w
    in
    let rec mk_words ind = if ind >= n then [] else mk_word ind :: mk_words (ind + 1) in
    mk_words 0
  ;;
    ...
    | (ps, _) :: _ ->
      let sets =
        List.fold_left
          ...
          (List.map (fun _ -> NativeSet.empty) ps)
          cases
      in
  ...
  let count_arities_first cases =
    let set =
      List.fold_left
        (fun set case ->
          match case with p :: _, _ -> NativeSet.add p set | _ -> assert false)
        NativeSet.empty
        cases
    in
  ...
  let best_col =
    let rec do_rec kbest best k = function
      | [] -> kbest
      | x :: xs ->
        if x < best then do_rec k x (k + 1) xs else do_rec kbest best (k + 1) xs
    in
  ...
  let swap k idxs cases =
    ...
    then idxs, cases
      ...
      and cases = List.map (fun (ps, act) -> swap_list k ps, act) cases in
      if dbg then pp_match stderr "SWAP" idxs cases;
      idxs, cases)
  ;;

  let best_first idxs cases =
    match idxs with
  ...
  module Divide (O : Set.OrderedType) = struct
    ...
    let divide cases =
      let env =
        List.fold_left
          (fun env (p, psact) ->
            let old = try OMap.find p env with Not_found -> [] in
  ...
  let by_cell cases =
    DivideNative.divide
      (List.map
         (fun case ->
           match case with p :: ps, act -> p, (ps, act) | [], _ -> assert false)
         cases)
  ;;

  (* Split into two halves *)

  let rec do_split idx env =
    match env with
    | [] -> assert false
    | ((midkey, _) as x) :: rem ->
      if idx <= 0
 ...
 *)
  let match_oncell compile_rec str default idx env =
    ...
    let rec comp_rec env =
      ...
      then
        List.fold_right
          (fun (key, cases) ifnot -> mk_eq id key (compile_rec str default cases) ifnot)
          env
          default
      else (
        let lt, midkey, ge = split_env len env in
        mk_lt id midkey (comp_rec lt) (comp_rec ge))
    in
 ...
 *)
  ...
  let rec do_compile_pats idxs str default cases =
    if dbg then pp_match stderr "COMPILE" idxs cases;
    match idxs with
    | [] -> (match cases with [] -> default | (_, e) :: _ -> e)
    | _ :: _ ->
      let idxs, cases = best_first idxs cases in
      (match idxs with
      | [] -> assert false
      | idx :: idxs -> match_oncell (do_compile_pats idxs) str default idx (by_cell cases))
  ;;
  ...
  let by_size cases =
    DivideInt.divide (List.map (fun ((ps, _) as case) -> List.length ps, case) cases)
  ;;
    ...
    let size_cases =
      List.map
        (fun (len, cases) ->
          ...
          let act = do_compile_pats (interval from_ind len) str default cases in
          len, act)
        (by_size cases)
    in
 ...
 *)
  let top_compile debuginfo str default cases =
    ...
    then (
      ...
      let compile_size_rest str default cases =
        compile_by_size debuginfo 1 str default cases
      in
  ...
  let compile dbg str default cases =
    ...
    let cases, default =
      ...
      | (_, e) :: cases, None | cases, Some e -> cases, e
      | [], None -> assert false
    in
    let cases = List.rev_map (fun (s, act) -> pat_of_string s, act) cases in
