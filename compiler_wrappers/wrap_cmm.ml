...
let typ_addr = Some Addr
let typ_int = Some Int
let typ_float = Some Float
...
let ge_component comp1 comp2 =
  ...
  | Addr, Val -> true
  | Float, Float -> true
  | (Int | Addr | Val), Float | Float, (Int | Addr | Val) -> assert false
;;
...
type expression =
  ...
  | Cifthenelse of
      expression * Debuginfo.t * expression * Debuginfo.t * expression * Debuginfo.t
  | Cswitch of expression * int array * (expression * Debuginfo.t) array * Debuginfo.t
  | Ccatch of
      rec_flag
      * (int * (Backend_var.With_provenance.t * machtype) list * expression * Debuginfo.t)
        list
      * expression
...
let iter_shallow_tail f = function
  ...
  | Cconst_natpointer _ | Cblockheader _ | Cvar _ | Cassign _ | Ctuple _ | Cop _ ->
    false
;;
...
let rec map_tail f = function
  ...
  | ( Cconst_int _ | Cconst_natint _ | Cconst_float _ | Cconst_symbol _ | Cconst_pointer _
    | Cconst_natpointer _ | Cblockheader _ | Cvar _ | Cassign _ | Ctuple _ | Cop _ ) as c
    ->
    f c
;;
