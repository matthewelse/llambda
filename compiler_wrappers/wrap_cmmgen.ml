...
open Wrap_cmmhelpers

(* Environments used for translation to Wrap_cmm. *)
...
type env =
  ...
  ; notify_catch : (Wrap_cmm.expression list -> unit) IntMap.t
...
let empty_env =
  { unboxed_ids = V.empty; notify_catch = IntMap.empty; environment_param = None }
;;

let create_env ~environment_param = { empty_env with environment_param }

let is_unboxed_id id env =
  try Some (V.find_same id env.unboxed_ids) with Not_found -> None
;;

let add_unboxed_id id unboxed_id bn env =
  { env with unboxed_ids = V.add id (unboxed_id, bn) env.unboxed_ids }
;;

let add_notify_catch n f env = { env with notify_catch = IntMap.add n f env.notify_catch }

let notify_catch i env l =
  match IntMap.find_opt i env.notify_catch with Some f -> f l | None -> ()
;;
...
let mut_from_env env ptr =
  ...
  | Some environment_param ->
    ...
    | Cvar ptr ->
      ...
      if V.same environment_param ptr then Immutable else Mutable
    | _ -> Mutable)
;;
...
let rec expr_size env = function
  | Uvar id -> (try V.find_same id env with Not_found -> RHS_nonrec)
  ...
  | Uletrec (bindings, body) ->
    ...
    in
    expr_size env body
  | Uprim (Pmakeblock _, args, _) -> RHS_block (List.length args)
  | Uprim (Pmakearray ((Paddrarray | Pintarray), _), args, _) ->
    RHS_block (List.length args)
  | Uprim (Pmakearray (Pfloatarray, _), args, _) -> RHS_floatblock (List.length args)
  | Uprim (Pmakearray (Pgenarray, _), _, _) ->
    (* Pgenarray is excluded from recursive bindings by the
        check in Translcore.check_recursive_lambda *)
    RHS_nonrec
  | Uprim (Pduprecord ((Record_regular | Record_inlined _), sz), _, _) -> RHS_block sz
  | Uprim (Pduprecord (Record_unboxed _, _), _, _) -> assert false
  | Uprim (Pduprecord (Record_extension _, sz), _, _) -> RHS_block (sz + 1)
  | Uprim (Pduprecord (Record_float, sz), _, _) -> RHS_floatblock sz
  ...
  | Usequence (_exp, exp') -> expr_size env exp'
  | Uoffset (exp, offset) ->
...
(* Translate structured constants to Wrap_cmm data items *)

let transl_constant dbg = function
  | Uconst_int n -> int_const dbg n
  | Uconst_ptr n ->
  ...
  | Uconst_ref (label, _) -> Cconst_symbol (label, dbg)
;;

let emit_constant cst cont =
  match cst with
  | Uconst_int n | Uconst_ptr n -> cint_const n :: cont
  | Uconst_ref (sym, _) -> Csymbol_address sym :: cont
;;

let emit_structured_constant ((_sym, is_global) as symb) cst cont =
  match cst with
  | Uconst_float s -> emit_float_constant symb s cont
  | Uconst_string s -> emit_string_constant symb s cont
  | Uconst_int32 n -> emit_int32_constant symb n cont
  | Uconst_int64 n -> emit_int64_constant symb n cont
  | Uconst_nativeint n -> emit_nativeint_constant symb n cont
  ...
  | Uconst_float_array fields -> emit_float_array_constant symb fields cont
  | Uconst_closure (fundecls, lbl, fv) ->
    Wrap_cmmgen_state.add_constant lbl (Const_closure (is_global, fundecls, fv));
    List.iter (fun f -> Wrap_cmmgen_state.add_function f) fundecls;
    cont
;;
...
let box_int_constant sym bi n =
  match bi with
  | Pnativeint -> emit_nativeint_constant (sym, Local) n []
  | Pint32 ->
...
let box_int dbg bi arg =
  ...
  | Cconst_int (n, _) ->
    ...
    Wrap_cmmgen_state.add_data_items data_items;
  ...
  | Cconst_natint (n, _) ->
    ...
    Wrap_cmmgen_state.add_data_items data_items;
    Cconst_symbol (sym, dbg)
  | _ -> box_int_gen dbg bi arg
;;

(* Boxed numbers *)

let typ_of_boxed_number = function
  | Boxed_float _ -> Wrap_cmm.typ_float
  | Boxed_integer _ -> Wrap_cmm.typ_int
;;
...
let equal_boxed_number bn1 bn2 =
  ...
  | Boxed_integer (ui1, _), Boxed_integer (ui2, _) -> equal_unboxed_integer ui1 ui2
  | _, _ -> false
;;
...
let unbox_number dbg bn arg =
  match bn with
  | Boxed_float dbg -> unbox_float dbg arg
  | Boxed_integer (Pint32, _) -> low_32 dbg (unbox_int dbg Pint32 arg)
...
(* expression never returns a result *)
...
let join_unboxed_number_kind ~strict k1 k2 =
  match k1, k2 with
  | Boxed (b1, c1), Boxed (b2, c2) when equal_boxed_number b1 b2 -> Boxed (b1, c1 && c2)
  | No_result, k | k, No_result ->
    k (* if a branch never returns, it is safe to unbox it *)
  | (No_unboxing, k | k, No_unboxing) when not strict -> k
  | _, _ -> No_unboxing
;;

let is_unboxed_number_Wrap_cmm ~strict cmm =
  let r = ref No_result in
  let notify k = r := join_unboxed_number_kind ~strict !r k in
  let rec aux = function
    ...
    | Cop (Calloc, [ Cblockheader (hdr, _); Cconst_symbol (ops, _); _ ], dbg) ->
      if Nativeint.equal hdr boxedintnat_header && String.equal ops caml_nativeint_ops
      then notify (Boxed (Boxed_integer (Pnativeint, dbg), false))
      else if Nativeint.equal hdr boxedint32_header && String.equal ops caml_int32_ops
      then notify (Boxed (Boxed_integer (Pint32, dbg), false))
      else if Nativeint.equal hdr boxedint64_header && String.equal ops caml_int64_ops
      then notify (Boxed (Boxed_integer (Pint64, dbg), false))
      else notify No_unboxing
    | Cconst_symbol (s, _) ->
      (match Wrap_cmmgen_state.structured_constant_of_sym s with
      | Some (Uconst_float _) -> notify (Boxed (Boxed_float Debuginfo.none, true))
      ...
      | _ -> notify No_unboxing)
    | l -> if not (Wrap_cmm.iter_shallow_tail aux l) then notify No_unboxing
  in
  aux cmm;
  !r
;;
...
let rec transl env e =
  ...
  | Uconst sc -> transl_constant Debuginfo.none sc
  | Uclosure (fundecls, []) ->
    let sym = Compilenv.new_const_symbol () in
    Wrap_cmmgen_state.add_constant sym (Const_closure (Local, fundecls, []));
    List.iter (fun f -> Wrap_cmmgen_state.add_function f) fundecls;
    let dbg = match fundecls with [] -> Debuginfo.none | fundecl :: _ -> fundecl.dbg in
    Cconst_symbol (sym, dbg)
  | Uclosure (fundecls, clos_vars) ->
    let rec transl_fundecls pos = function
      | [] -> List.map (transl env) clos_vars
      | f :: rem ->
        Wrap_cmmgen_state.add_function f;
        let dbg = f.dbg in
        let without_header =
        ...
        in
        if pos = 0 then without_header else alloc_infix_header pos f.dbg :: without_header
    in
    let dbg = match fundecls with [] -> Debuginfo.none | fundecl :: _ -> fundecl.dbg in
  ...
  | Ulet (str, kind, id, exp, body) -> transl_let env str kind id exp body
  | Uphantom_let (var, defining_expr, body) ->
    let defining_expr =
      ...
      | Some defining_expr ->
        let defining_expr =
          match defining_expr with
          | Uphantom_const (Uconst_ref (sym, _defining_expr)) -> Cphantom_const_symbol sym
          ...
          | Uphantom_read_field { var; field } -> Cphantom_read_field { var; field }
          | Uphantom_offset_var { var; offset_in_words } ->
            Cphantom_offset_var { var; offset_in_words }
          | Uphantom_block { tag; fields } -> Cphantom_block { tag; fields }
        in
        Some defining_expr
    in
    Cphantom_let (var, defining_expr, transl env body)
  | Uletrec (bindings, body) -> transl_letrec env bindings (transl env body)
  (* Primitives *)
  | Uprim (prim, args, dbg) ->
    (match simplif_primitive prim, args with
    | Pread_symbol sym, [] -> Cconst_symbol (sym, dbg)
    | Pmakeblock _, [] -> assert false
    | Pmakeblock (tag, _mut, _kind), args ->
      make_alloc dbg tag (List.map (transl env) args)
    | Pccall prim, args -> transl_ccall env prim args dbg
    ...
    | Pduparray _, [ arg ] ->
      let prim_obj_dup = Primitive.simple ~name:"caml_obj_dup" ~arity:1 ~alloc:true in
      transl_ccall env prim_obj_dup [ arg ] dbg
    | Pmakearray _, [] -> Misc.fatal_error "Pmakearray is not allowed for an empty array"
    ...
    | Pbigarrayref (unsafe, _num_dims, elt_kind, layout), arg1 :: argl ->
      ...
      | Pbigarray_sint8 | Pbigarray_uint8 | Pbigarray_sint16 | Pbigarray_uint16 ->
        tag_int elt dbg
    ...
    | Pbigarrayset (unsafe, _num_dims, elt_kind, layout), arg1 :: argl ->
      ...
      return_unit
        ...
        (bigarray_set
           ...
           (match elt_kind with
           | Pbigarray_float32 | Pbigarray_float64 -> transl_unbox_float dbg env argnewval
           ...
           | Pbigarray_native_int -> transl_unbox_int dbg env Pnativeint argnewval
           | Pbigarray_caml_int -> untag_int (transl env argnewval) dbg
           | Pbigarray_sint8 | Pbigarray_uint8 | Pbigarray_sint16 | Pbigarray_uint16 ->
    ...
    | Pbigarraydim n, [ b ] ->
      let dim_ofs = 4 + n in
      tag_int
        (Cop (Cload (Word_int, Mutable), [ field_address (transl env b) dim_ofs dbg ], dbg))
        dbg
    | p, [ arg ] -> transl_prim_1 env p arg dbg
    | p, [ arg1; arg2 ] -> transl_prim_2 env p arg1 arg2 dbg
    | p, [ arg1; arg2; arg3 ] -> transl_prim_3 env p arg1 arg2 arg3 dbg
    | Pread_symbol _, _ :: _ :: _ :: _ :: _
    | Pbigarrayset (_, _, _, _), []
    | Pbigarrayref (_, _, _, _), []
    | (Pbigarraydim _ | Pduparray (_, _)), ([] | _ :: _ :: _ :: _ :: _) ->
      fatal_error "Wrap_cmmgen.transl:prim, wrong arity"
    | ( ( Pfield_computed | Psequand | Psequor | Pnot | Pnegint | Paddint | Psubint
        | Pmulint | Pandint | Porint | Pxorint | Plslint | Plsrint | Pasrint | Pintoffloat
        | Pfloatofint | Pnegfloat | Pabsfloat | Paddfloat | Psubfloat | Pmulfloat
        | Pdivfloat | Pstringlength | Pstringrefu | Pstringrefs | Pbyteslength
        | Pbytesrefu | Pbytessetu | Pbytesrefs | Pbytessets | Pisint | Pisout | Pbswap16
        | Pint_as_pointer | Popaque | Pfield _
        | Psetfield (_, _, _)
        | Psetfield_computed (_, _)
        | Pfloatfield _
        | Psetfloatfield (_, _)
        | Pduprecord (_, _)
        | Praise _ | Pdivint _ | Pmodint _ | Pintcomp _ | Poffsetint _ | Pcompare_ints
        | Pcompare_floats | Pcompare_bints _ | Poffsetref _ | Pfloatcomp _
        ...
        | Pbigstring_set _ | Pbbswap _ )
      , _ ) ->
      fatal_error "Wrap_cmmgen.transl:prim")
  (* Control structures *)
  | Uswitch (arg, s, dbg) ->
    (* As in the bytecode interpreter, only matching against constants
         can be checked *)
    if Array.length s.us_index_blocks = 0
    then
      make_switch
        ...
        dbg
    else if Array.length s.us_index_consts = 0
    then
      bind "switch" (transl env arg) (fun arg ->
          transl_switch dbg env (get_tag arg dbg) s.us_index_blocks s.us_actions_blocks)
    else
      bind "switch" (transl env arg) (fun arg ->
          Cifthenelse
  ...
  | Uifthenelse (cond, ifso, ifnot) ->
    ...
    let dbg = Debuginfo.none in
    transl_if env Unknown dbg cond ifso_dbg (transl env ifso) ifnot_dbg (transl env ifnot)
  | Usequence (exp1, exp2) -> Csequence (remove_unit (transl env exp1), transl env exp2)
  | Uwhile (cond, body) ->
    let dbg = Debuginfo.none in
  ...
  | Uassign (id, exp) ->
    ...
    (match is_unboxed_id id env with
    | None -> return_unit dbg (Cassign (id, cexp))
    | Some (unboxed_id, bn) ->
...
and transl_catch env nfail ids body handler dbg =
  ...
  let report args =
    List.iter2
      (fun (_id, kind, u) c ->
        ...
        in
        u := join_unboxed_number_kind ~strict !u (is_unboxed_number_Wrap_cmm ~strict c))
      ids
      args
  in
    ...
    List.fold_right
      (fun (id, _kind, u) (env, rewrite, ids) ->
        ...
        | No_unboxing | Boxed (_, true) | No_result ->
          env, (fun x -> x) :: rewrite, (id, Wrap_cmm.typ_val) :: ids
        | Boxed (bn, false) ->
  ...
  then (* No unboxing *)
    ...
    let body =
      ...
      let rec aux e =
        match Wrap_cmm.map_shallow aux e with
...
and transl_make_array dbg env kind args =
  ...
  | Pgenarray ->
    Cop
      ( Cextcall ("caml_make_array", typ_val, true, None)
      , [ make_alloc dbg 0 (List.map (transl env) args) ]
      , dbg )
  | Paddrarray | Pintarray -> make_alloc dbg 0 (List.map (transl env) args)
  | Pfloatarray ->
    make_float_alloc dbg Obj.double_array_tag (List.map (transl_unbox_float dbg env) args)
...
and transl_ccall env prim args dbg =
  ...
  wrap_result
    (Cop (Cextcall (Primitive.native_name prim, typ_res, prim.prim_alloc, None), args, dbg))

and transl_prim_1 env p arg dbg =
  match p with
  (* Generic operations *)
  | Popaque -> transl env arg
  (* Heap operations *)
  | Pfield n -> get_field env (transl env arg) n dbg
  | Pfloatfield n ->
    let ptr = transl env arg in
    box_float dbg (floatfield n ptr dbg)
  | Pint_as_pointer -> int_as_pointer (transl env arg) dbg
  (* Exceptions *)
  | Praise rkind -> raise_prim rkind (transl env arg) dbg
  (* Integer operations *)
  | Pnegint -> negint (transl env arg) dbg
  | Poffsetint n -> offsetint n (transl env arg) dbg
  | Poffsetref n -> offsetref n (transl env arg) dbg
  ...
  | Pnegfloat -> box_float dbg (Cop (Cnegf, [ transl_unbox_float dbg env arg ], dbg))
  | Pabsfloat -> box_float dbg (Cop (Cabsf, [ transl_unbox_float dbg env arg ], dbg))
  (* String operations *)
  | Pstringlength | Pbyteslength -> tag_int (string_length (transl env arg) dbg) dbg
  (* Array operations *)
  | Parraylength kind -> arraylength kind (transl env arg) dbg
  (* Boolean operations *)
  | Pnot ->
    transl_if
      env
      ...
      arg
      dbg
      (Cconst_pointer (1, dbg))
      dbg
      (Cconst_pointer (3, dbg))
  (* Test integer/block *)
  | Pisint -> tag_int (Cop (Cand, [ transl env arg; Cconst_int (1, dbg) ], dbg)) dbg
  (* Boxed integers *)
  | Pbintofint bi -> box_int dbg bi (untag_int (transl env arg) dbg)
  | Pintofbint bi -> tag_int (transl_unbox_int dbg env bi arg) dbg
  | Pcvtbint (bi1, bi2) -> box_int dbg bi2 (transl_unbox_int dbg env bi1 arg)
  | Pnegbint bi ->
    box_int
      dbg
      bi
      (Cop (Csubi, [ Cconst_int (0, dbg); transl_unbox_int dbg env bi arg ], dbg))
  | Pbbswap bi -> box_int dbg bi (bbswap bi (transl_unbox_int dbg env bi arg) dbg)
  | Pbswap16 ->
    tag_int (bswap16 (ignore_high_bit_int (untag_int (transl env arg) dbg)) dbg) dbg
  | Pfield_computed | Psequand | Psequor | Paddint | Psubint | Pmulint | Pandint | Porint
  | Pxorint | Plslint | Plsrint | Pasrint | Paddfloat | Psubfloat | Pmulfloat | Pdivfloat
  | Pstringrefu | Pstringrefs | Pbytesrefu | Pbytessetu | Pbytesrefs | Pbytessets | Pisout
  | Pread_symbol _
  | Pmakeblock (_, _, _)
  | Psetfield (_, _, _)
  | Psetfield_computed (_, _)
  | Psetfloatfield (_, _)
  | Pduprecord (_, _)
  | Pccall _ | Pdivint _ | Pmodint _ | Pintcomp _ | Pfloatcomp _
  | Pmakearray (_, _)
  | Pcompare_ints | Pcompare_floats | Pcompare_bints _
  | Pduparray (_, _)
  | Parrayrefu _ | Parraysetu _ | Parrayrefs _ | Parraysets _ | Paddbint _ | Psubbint _
  | Pmulbint _ | Pdivbint _ | Pmodbint _ | Pandbint _ | Porbint _ | Pxorbint _
  | Plslbint _ | Plsrbint _ | Pasrbint _
  | Pbintcomp (_, _)
  | Pbigarrayref (_, _, _, _)
  | Pbigarrayset (_, _, _, _)
  | Pbigarraydim _ | Pstring_load _ | Pbytes_load _ | Pbytes_set _ | Pbigstring_load _
  | Pbigstring_set _ ->
    fatal_errorf "Wrap_cmmgen.transl_prim_1: %a" Printclambda_primitives.primitive p
...
and transl_prim_2 env p arg1 arg2 dbg =
  ...
  | Psequor ->
    ...
    transl_sequor
      ...
      (Cconst_pointer (1, dbg))
  (* Integer operations *)
  | Paddint -> add_int_caml (transl env arg1) (transl env arg2) dbg
  | Psubint -> sub_int_caml (transl env arg1) (transl env arg2) dbg
  | Pmulint -> mul_int_caml (transl env arg1) (transl env arg2) dbg
  ...
  | Porint -> or_int_caml (transl env arg1) (transl env arg2) dbg
  | Pxorint -> xor_int_caml (transl env arg1) (transl env arg2) dbg
  | Plslint -> lsl_int_caml (transl env arg1) (transl env arg2) dbg
  ...
  | Paddfloat ->
    box_float
      dbg
      (Cop
         (Caddf, [ transl_unbox_float dbg env arg1; transl_unbox_float dbg env arg2 ], dbg))
  | Psubfloat ->
    box_float
      dbg
      (Cop
         (Csubf, [ transl_unbox_float dbg env arg1; transl_unbox_float dbg env arg2 ], dbg))
  | Pmulfloat ->
    box_float
      dbg
      (Cop
         (Cmulf, [ transl_unbox_float dbg env arg1; transl_unbox_float dbg env arg2 ], dbg))
  | Pdivfloat ->
    box_float
      dbg
      (Cop
         (Cdivf, [ transl_unbox_float dbg env arg1; transl_unbox_float dbg env arg2 ], dbg))
  | Pfloatcomp cmp ->
    tag_int
      (Cop
         ( Ccmpf cmp
  ...
  | Pstringrefs | Pbytesrefs -> stringref_safe (transl env arg1) (transl env arg2) dbg
  ...
  (* Array operations *)
  | Parrayrefu kind -> arrayref_unsafe kind (transl env arg1) (transl env arg2) dbg
  ...
  | Paddbint bi ->
    box_int
      ...
      (Cop
         ( Caddi
         , [ transl_unbox_int_low dbg env bi arg1; transl_unbox_int_low dbg env bi arg2 ]
         , dbg ))
  | Psubbint bi ->
    box_int
      dbg
  ...
  | Plslbint bi ->
    box_int
      ...
      (Cop
         ( Clsl
         , [ transl_unbox_int_low dbg env bi arg1; untag_int (transl env arg2) dbg ]
         , dbg ))
  | Plsrbint bi ->
    box_int
      dbg
  ...
  | Pasrbint bi ->
    box_int
      ...
      bi
      (Cop
         (Casr, [ transl_unbox_int dbg env bi arg1; untag_int (transl env arg2) dbg ], dbg))
  | Pbintcomp (bi, cmp) ->
    tag_int
      (Cop
         ( Ccmpi cmp
         , [ transl_unbox_int dbg env bi arg1; transl_unbox_int dbg env bi arg2 ]
         , dbg ))
      dbg
  | Pnot | Pnegint | Pintoffloat | Pfloatofint | Pnegfloat | Pabsfloat | Pstringlength
  | Pbyteslength | Pbytessetu | Pbytessets | Pisint | Pbswap16 | Pint_as_pointer | Popaque
  | Pread_symbol _
  | Pmakeblock (_, _, _)
  | Pfield _
  | Psetfield_computed (_, _)
  | Pfloatfield _
  | Pduprecord (_, _)
  | Pccall _ | Praise _ | Poffsetint _ | Poffsetref _
  | Pmakearray (_, _)
  | Pduparray (_, _)
  | Parraylength _ | Parraysetu _ | Parraysets _ | Pbintofint _ | Pintofbint _
  | Pcvtbint (_, _)
  | Pnegbint _
  | Pbigarrayref (_, _, _, _)
  | Pbigarrayset (_, _, _, _)
  | Pbigarraydim _ | Pbytes_set _ | Pbigstring_set _ | Pbbswap _ ->
    fatal_errorf "Wrap_cmmgen.transl_prim_2: %a" Printclambda_primitives.primitive p

and transl_prim_3 env p arg1 arg2 arg3 dbg =
  match p with
  (* Heap operations *)
  | Psetfield_computed (ptr, init) ->
    setfield_computed ptr init (transl env arg1) (transl env arg2) (transl env arg3) dbg
  (* String operations *)
  | Pbytessetu ->
    bytesset_unsafe (transl env arg1) (transl env arg2) (transl env arg3) dbg
  | Pbytessets -> bytesset_safe (transl env arg1) (transl env arg2) (transl env arg3) dbg
  (* Array operations *)
  | Parraysetu kind ->
    let newval =
      match kind with
  ...
  | Pbytes_set (size, unsafe) ->
    bytes_set
      ...
      (transl env arg1)
      (transl env arg2)
      (transl_unbox_sized size dbg env arg3)
      dbg
  | Pbigstring_set (size, unsafe) ->
    bigstring_set
      ...
      (transl_unbox_sized size dbg env arg3)
      dbg
  | Pfield_computed | Psequand | Psequor | Pnot | Pnegint | Paddint | Psubint | Pmulint
  | Pandint | Porint | Pxorint | Plslint | Plsrint | Pasrint | Pintoffloat | Pfloatofint
  | Pnegfloat | Pabsfloat | Paddfloat | Psubfloat | Pmulfloat | Pdivfloat | Pstringlength
  | Pstringrefu | Pstringrefs | Pbyteslength | Pbytesrefu | Pbytesrefs | Pisint | Pisout
  ...
  | Psetfloatfield (_, _)
  | Pduprecord (_, _)
  | Pccall _ | Praise _ | Pdivint _ | Pmodint _ | Pintcomp _ | Pcompare_ints
  | Pcompare_floats | Pcompare_bints _ | Poffsetint _ | Poffsetref _ | Pfloatcomp _
  | Pmakearray (_, _)
  | Pduparray (_, _)
  | Parraylength _ | Parrayrefu _ | Parrayrefs _ | Pbintofint _ | Pintofbint _
  | Pcvtbint (_, _)
  | Pnegbint _ | Paddbint _ | Psubbint _ | Pmulbint _ | Pdivbint _ | Pmodbint _
  | Pandbint _ | Porbint _ | Pxorbint _ | Plslbint _ | Plsrbint _ | Pasrbint _
  | Pbintcomp (_, _)
  | Pbigarrayref (_, _, _, _)
  | Pbigarrayset (_, _, _, _)
  | Pbigarraydim _ | Pstring_load _ | Pbytes_load _ | Pbigstring_load _ | Pbbswap _ ->
    fatal_errorf "Wrap_cmmgen.transl_prim_3: %a" Printclambda_primitives.primitive p

and transl_unbox_float dbg env exp = unbox_float dbg (transl env exp)
and transl_unbox_int dbg env bi exp = unbox_int dbg bi (transl env exp)
...
and transl_unbox_sized size dbg env exp =
  match size with
  | Sixteen -> ignore_high_bit_int (untag_int (transl env exp) dbg)
...
and transl_let env str kind id exp body =
  ...
  let unboxing =
    ...
    match str, kind with
    | Mutable, Pfloatval -> Boxed (Boxed_float dbg, false)
    | Mutable, Pboxedintval bi -> Boxed (Boxed_integer (bi, dbg), false)
    | _, (Pfloatval | Pboxedintval _) ->
      ...
      is_unboxed_number_Wrap_cmm ~strict:false cexp
    | _, Pgenval ->
      ...
      is_unboxed_number_Wrap_cmm ~strict:true cexp
    | _, Pintval -> No_unboxing
  in
  match unboxing with
  | No_unboxing | Boxed (_, true) | No_result ->
  ...
  | Boxed (boxed_number, false) ->
    ...
    let body = transl (add_unboxed_id (VP.var id) unboxed_id boxed_number env) body in
    (match str, boxed_number with
...
and is_shareable_cont exp = match exp with Cexit (_, []) -> true | _ -> false

and make_shareable_cont dbg mk exp =
  if is_shareable_cont exp
  then mk exp
  else (
    let nfail = next_raise_count () in
    make_catch nfail (mk (Cexit (nfail, []))) exp dbg)
...
and transl_if
  ...
  | Uifthenelse (arg1, arg2, Uconst (Uconst_ptr 0)) ->
    ...
    let ifso_dbg = Debuginfo.none in
    transl_sequand env approx inner_dbg arg1 ifso_dbg arg2 then_dbg then_ else_dbg else_
  | Uprim (Psequand, [ arg1; arg2 ], inner_dbg) ->
    transl_sequand env approx inner_dbg arg1 inner_dbg arg2 then_dbg then_ else_dbg else_
  | Uifthenelse (arg1, Uconst (Uconst_ptr 1), arg2) ->
    let inner_dbg = Debuginfo.none in
    let ifnot_dbg = Debuginfo.none in
    transl_sequor env approx inner_dbg arg1 ifnot_dbg arg2 then_dbg then_ else_dbg else_
  | Uprim (Psequor, [ arg1; arg2 ], inner_dbg) ->
    transl_sequor env approx inner_dbg arg1 inner_dbg arg2 then_dbg then_ else_dbg else_
  | Uprim (Pnot, [ arg ], _dbg) ->
    transl_if env (invert_then_else approx) dbg arg else_dbg else_ then_dbg then_
  | Uifthenelse (Uconst (Uconst_ptr 1), ifso, _) ->
    let ifso_dbg = Debuginfo.none in
    transl_if env approx ifso_dbg ifso then_dbg then_ else_dbg else_
  | Uifthenelse (Uconst (Uconst_ptr 0), _, ifnot) ->
    let ifnot_dbg = Debuginfo.none in
    transl_if env approx ifnot_dbg ifnot then_dbg then_ else_dbg else_
  | Uifthenelse (cond, ifso, ifnot) ->
    let inner_dbg = Debuginfo.none in
    let ifso_dbg = Debuginfo.none in
    let ifnot_dbg = Debuginfo.none in
      ...
      (fun shareable_then ->
        make_shareable_cont
          ...
          (fun shareable_else ->
            mk_if_then_else
              inner_dbg
              (test_bool inner_dbg (transl env cond))
              ifso_dbg
              (transl_if
                 env
                 approx
  ...
  | _ ->
    (match approx with
    | Then_true_else_false -> transl env cond
    | Then_false_else_true -> mk_not dbg (transl env cond)
    | Unknown ->
      mk_if_then_else dbg (test_bool dbg (transl env cond)) then_dbg then_ else_dbg else_)

and transl_sequand
    env
    (approx : then_else)
    (arg1_dbg : Debuginfo.t)
    arg1
    (arg2_dbg : Debuginfo.t)
    arg2
    (then_dbg : Debuginfo.t)
    then_
    (else_dbg : Debuginfo.t)
  ...
  make_shareable_cont
    ...
    (fun shareable_else ->
      transl_if
        ...
        arg1
        arg2_dbg
        (transl_if env approx arg2_dbg arg2 then_dbg then_ else_dbg shareable_else)
        else_dbg
        shareable_else)
    else_
  ...
  make_shareable_cont
    ...
    (fun shareable_then ->
      transl_if
        ...
        shareable_then
        arg2_dbg
        (transl_if env approx arg2_dbg arg2 then_dbg shareable_then else_dbg else_))
    then_

(* This assumes that [arg] can be safely discarded if it is not used. *)
and transl_switch dbg env arg index cases =
  match Array.length cases with
  | 0 -> fatal_error "Wrap_cmmgen.transl_switch"
...
and transl_letrec env bindings cont =
  let dbg = Debuginfo.none in
  let bsz = List.map (fun (id, exp) -> id, exp, expr_size V.empty exp) bindings in
  let op_alloc prim args = Cop (Cextcall (prim, typ_val, true, None), args, dbg) in
  let rec init_blocks = function
    | [] -> fill_nonrec bsz
    | (id, _exp, RHS_block sz) :: rem ->
      Clet (id, op_alloc "caml_alloc_dummy" [ int_const dbg sz ], init_blocks rem)
    ...
    | (id, _exp, RHS_floatblock sz) :: rem ->
      Clet (id, op_alloc "caml_alloc_dummy_float" [ int_const dbg sz ], init_blocks rem)
    | (id, _exp, RHS_nonrec) :: rem -> Clet (id, Cconst_int (1, dbg), init_blocks rem)
  and fill_nonrec = function
    | [] -> fill_blocks bsz
    | (_id, _exp, (RHS_block _ | RHS_infix _ | RHS_floatblock _)) :: rem ->
      fill_nonrec rem
    | (id, exp, RHS_nonrec) :: rem -> Clet (id, transl env exp, fill_nonrec rem)
  and fill_blocks = function
    | [] -> cont
    | (id, exp, (RHS_block _ | RHS_infix _ | RHS_floatblock _)) :: rem ->
      let op =
        Cop
          ( Cextcall ("caml_update_dummy", typ_void, false, None)
          , [ Cvar (VP.var id); transl env exp ]
          , dbg )
      in
      Csequence (op, fill_blocks rem)
    | (_id, _exp, RHS_nonrec) :: rem -> fill_blocks rem
  in
  init_blocks bsz
;;
...
let transl_function f =
  ...
  let cmm_body =
    ...
    (* TODO melse: enable AFL instrumentation with LLVM *)
    transl env body
  in
  let fun_codegen_options =
    if !Clflags.optimize_for_speed then [] else [ Reduce_code_size ]
  in
  Cfunction
    { fun_name = f.label
...
let rec transl_all_functions already_translated cont =
  match Wrap_cmmgen_state.next_function () with
  | None -> cont, already_translated
  | Some f ->
    let sym = f.label in
    if String.Set.mem sym already_translated
    then transl_all_functions already_translated cont
    else
...
let transl_clambda_constants (constants : Clambda.preallocated_constant list) cont =
  let c = ref cont in
  ...
  List.iter
    (fun { symbol; exported; definition = cst; provenance = _ } ->
      let global : Wrap_cmmgen_state.is_global = if exported then Global else Local in
...
let emit_cmm_data_items_for_constants cont =
  let c = ref cont in
  String.Map.iter
    (fun symbol (cst : Wrap_cmmgen_state.constant) ->
      match cst with
    ...
    (Wrap_cmmgen_state.get_and_clear_constants ());
  Cdata (Wrap_cmmgen_state.get_and_clear_data_items ()) :: !c
;;

let transl_all_functions cont =
  let rec aux already_translated cont translated_functions =
    if Wrap_cmmgen_state.no_more_functions ()
  ...
  in
  let cont, translated_functions = aux String.Set.empty cont [] in
  let translated_functions =
    (* Sort functions according to source position *)
    List.map
      snd
      (List.sort
         (fun (dbg1, _) (dbg2, _) -> Debuginfo.compare dbg1 dbg2)
...
let compunit (ulam, preallocated_blocks, constants) =
  assert (Wrap_cmmgen_state.no_more_functions ());
  Wrap_cmmgen_state.set_structured_constants constants;
  let init_code =
    (* TODO melse: AFL instrument? *)
    transl empty_env ulam
  in
  let c1 =
    [ Cfunction
        { fun_name = Compilenv.make_symbol (Some "entry")
        ; fun_args = []
        ; fun_body = init_code
  ...
  Wrap_cmmgen_state.set_structured_constants [];
