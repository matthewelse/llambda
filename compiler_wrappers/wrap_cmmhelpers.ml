...
let bind name arg fn =
  match arg with
  | Cvar _ | Cconst_int _ | Cconst_natint _ | Cconst_symbol _ | Cconst_pointer _
  | Cconst_natpointer _ | Cblockheader _ ->
    fn arg
  | _ ->
...
let ignore_high_bit_int = function
  | Cop (Casr, [ Cop (Clsl, [ c; Cconst_int (1, _) ], _); Cconst_int (1, _) ], _) -> c
  | c -> c
;;
...
let mk_if_then_else dbg cond ifso_dbg ifso ifnot_dbg ifnot =
  ...
  | Cconst_int (1, _) -> ifso
  | _ -> Cifthenelse (cond, ifso_dbg, ifso, ifnot_dbg, ifnot, dbg)
;;
...
let safe_div_bi is_safe =
  safe_divmod_bi div_int is_safe (fun c1 dbg ->
...
let unbox_float dbg =
  map_tail (function
      ...
      | Cconst_symbol (s, _dbg) as cmm ->
        ...
        | _ -> Cop (Cload (Double_u, Immutable), [ cmm ], dbg))
...
let rec remove_unit = function
  ...
  | Cifthenelse (cond, ifso_dbg, ifso, ifnot_dbg, ifnot, dbg) ->
    Cifthenelse (cond, ifso_dbg, remove_unit ifso, ifnot_dbg, remove_unit ifnot, dbg)
  | Cswitch (sel, index, cases, dbg) ->
    Cswitch (sel, index, Array.map (fun (case, dbg) -> remove_unit case, dbg) cases, dbg)
  ...
  | Cop (Capply _mty, args, dbg) -> Cop (Capply typ_void, args, dbg)
...
let array_indexing ?typ log2size ptr ofs dbg =
  let add =
    match typ with None | Some Addr -> Cadda | Some Int -> Caddi | _ -> assert false
  in
  match ofs with
  | Cconst_int (n, _) ->
    let i = n asr 1 in
...
let addr_array_ref arr ofs dbg =
  Cop (Cload (Word_val, Mutable), [ array_indexing log2_size_addr arr ofs dbg ], dbg)
;;

let int_array_ref arr ofs dbg =
  Cop (Cload (Word_int, Mutable), [ array_indexing log2_size_addr arr ofs dbg ], dbg)
;;

let unboxed_float_array_ref arr ofs dbg =
  Cop (Cload (Double_u, Mutable), [ array_indexing log2_size_float arr ofs dbg ], dbg)
;;

let float_array_ref arr ofs dbg = box_float dbg (unboxed_float_array_ref arr ofs dbg)

let addr_array_set arr ofs newval dbg =
  Cop
    ( Cextcall ("caml_modify", typ_void, false, None)
...
let addr_array_initialize arr ofs newval dbg =
  Cop
    ( Cextcall ("caml_initialize", typ_void, false, None)
    , [ array_indexing log2_size_addr arr ofs dbg; newval ]
    , dbg )
;;
...
let float_array_set arr ofs newval dbg =
  Cop
    ( Cstore (Double_u, Lambda.Assignment)
...
let lookup_tag obj tag dbg =
  bind "tag" tag (fun tag ->
      Cop (Cextcall ("caml_get_public_method", typ_val, false, None), [ obj; tag ], dbg))
;;
...
let make_alloc_generic set_fn dbg tag wordsize args =
  ...
  then Cop (Calloc, Cblockheader (block_header tag wordsize, dbg) :: args, dbg)
  else (
    ...
    let rec fill_fields idx = function
      ...
      | e1 :: el ->
        Csequence
          (set_fn (Cvar id) (Cconst_int (idx, dbg)) e1 dbg, fill_fields (idx + 2) el)
    in
    Clet
      ( VP.create id
      , Cop
          ( Cextcall ("caml_alloc", typ_val, true, None)
...
let apply_function_sym n =
  ...
  "caml_apply" ^ Int.to_string n
;;

let curry_function_sym n =
  Compilenv.need_curry_fun n;
  if n >= 0 then "caml_curry" ^ Int.to_string n else "caml_tuplify" ^ Int.to_string (-n)
;;
...
let bigarray_indexing unsafe elt_kind layout b args dbg =
  let check_ba_bound bound idx v = Csequence (make_checkbound dbg [ bound; idx ], v) in
  ...
  let rec ba_indexing dim_ofs delta_ofs = function
    ...
    | arg1 :: argl ->
      ...
      (* Load the untagged int bound for the given dimension *)
      let bound = Cop (Cload (Word_int, Mutable), [ field_address b dim_ofs dbg ], dbg) in
      if unsafe
      then add_int (mul_int (decr_int rem dbg) bound dbg) arg1 dbg
        ...
        bind "idx" arg1 (fun idx ->
            bind "bound" bound (fun bound ->
                ...
                let offset = add_int (mul_int (decr_int rem dbg) bound dbg) idx dbg in
  ...
  let offset =
    ...
    | Pbigarray_c_layout -> ba_indexing (4 + List.length args) (-1) (List.rev args)
    | Pbigarray_fortran_layout ->
      ba_indexing 5 1 (List.map (fun idx -> sub_int idx (Cconst_int (2, dbg)) dbg) args)
  and elt_size = bigarray_elt_size elt_kind in
...
let sign_extend_32 dbg e =
  ...
  then e
    ...
    Cop
      ( Casr
      , [ Cop (Clsl, [ low_32 dbg e; Cconst_int (32, dbg) ], dbg); Cconst_int (32, dbg) ]
      , dbg )
;;
...
let zero_extend_32 dbg e =
  if size_int = 4
  then e
  else Cop (Cand, [ low_32 dbg e; Cconst_natint (0xFFFFFFFFn, dbg) ], dbg)
;;
...
let alloc_matches_boxed_int bi ~hdr ~ops =
  ...
  | Pnativeint, Cblockheader (hdr, _dbg), Cconst_symbol (sym, _) ->
    Nativeint.equal hdr boxedintnat_header && String.equal sym caml_nativeint_ops
...
let unbox_int dbg bi =
  let default arg =
    ...
    then split_int64_for_32bit_target arg dbg
      ...
      Cop
        ( Cload ((if bi = Primitive.Pint32 then Thirtytwo_signed else Word_int), Immutable)
        , [ Cop (Cadda, [ arg; Cconst_int (size_addr, dbg) ], dbg) ]
        , dbg )
  in
  map_tail (function
      | Cop
          (Calloc, [ hdr; ops; Cop (Clsl, [ contents; Cconst_int (32, _) ], _dbg') ], _dbg)
      ...
      | Cop (Calloc, [ hdr; ops; contents ], _dbg)
        ...
        contents
      | Cconst_symbol (s, _dbg) as cmm ->
        (match Wrap_cmmgen_state.structured_constant_of_sym s, bi with
        | Some (Uconst_nativeint n), Primitive.Pnativeint -> Cconst_natint (n, dbg)
        | Some (Uconst_int32 n), Primitive.Pint32 ->
          Cconst_natint (Nativeint.of_int32 n, dbg)
        | Some (Uconst_int64 n), Primitive.Pint64 ->
          if size_int = 8
          then Cconst_natint (Int64.to_nativeint n, dbg)
          else (
            let low = Int64.to_nativeint n in
            let high = Int64.to_nativeint (Int64.shift_right_logical n 32) in
            if big_endian
            then Ctuple [ Cconst_natint (high, dbg); Cconst_natint (low, dbg) ]
            else Ctuple [ Cconst_natint (low, dbg); Cconst_natint (high, dbg) ])
        | _ -> default cmm)
      | cmm -> default cmm)
;;
...
let simplif_primitive_32bits
  ...
  | Pcvtbint (Pnativeint, Pint64) -> Pccall (default_prim "caml_int64_of_nativeint")
  | Pcvtbint (Pint64, Pnativeint) -> Pccall (default_prim "caml_int64_to_nativeint")
  | Pnegbint Pint64 -> Pccall (int64_native_prim "caml_int64_neg" 1 ~alloc:false)
  | Paddbint Pint64 -> Pccall (int64_native_prim "caml_int64_add" 2 ~alloc:false)
  | Psubbint Pint64 -> Pccall (int64_native_prim "caml_int64_sub" 2 ~alloc:false)
  | Pmulbint Pint64 -> Pccall (int64_native_prim "caml_int64_mul" 2 ~alloc:false)
  ...
  | Pandbint Pint64 -> Pccall (int64_native_prim "caml_int64_and" 2 ~alloc:false)
  | Porbint Pint64 -> Pccall (int64_native_prim "caml_int64_or" 2 ~alloc:false)
  | Pxorbint Pint64 -> Pccall (int64_native_prim "caml_int64_xor" 2 ~alloc:false)
...
let make_switch arg cases actions dbg =
  let extract_uconstant = function
    ...
    | (Cconst_int (n, _), _dbg | Cconst_pointer (n, _), _dbg) when n land 1 = 1 ->
      Some (Cint (Nativeint.of_int n))
    | (Cconst_natint (n, _), _dbg | Cconst_natpointer (n, _), _dbg)
  ...
  let make_table_lookup ~cases ~const_actions arg dbg =
    ...
    Wrap_cmmgen_state.add_constant
      table
      (Const_table
...
module SArgBlocks = struct
  ...
  let make_if cond ifso ifnot =
    Cifthenelse (cond, Debuginfo.none, ifso, Debuginfo.none, ifnot, Debuginfo.none)
  ;;
...
*)
      ...
      , fun body ->
          (match body with
          | Cexit (j, _) -> if i = j then handler else body
...
module StoreExpForSwitch = Switch.CtxStore (struct
  ...
  let make_key index expr =
    let continuation = match expr with Cexit (i, []) -> Some i | _ -> None in
...
module StoreExp = Switch.Store (struct
  ...
  let make_key = function Cexit (i, []) -> Some i | _ -> None
...
let transl_int_switch dbg arg low high cases default =
  ...
  | _ :: _ ->
    ...
    let cases = List.map (fun (i, act) -> i, store.Switch.act_store () act) cases in
    ...
    bind "switcher" arg (fun a ->
        SwitcherBlocks.zyva dbg (low, high) a (Array.of_list inters) store)
;;
...
let transl_switch_clambda loc arg index cases =
  ...
  | inters ->
    bind "switcher" arg (fun a ->
        SwitcherBlocks.zyva loc (0, n_index - 1) a (Array.of_list inters) store)
;;

let strmatch_compile =
  let module S =
    Strmatch.Make (struct
...
let direct_apply lbl args dbg = Cop (Capply typ_val, Cconst_symbol (lbl, dbg) :: args, dbg)

let generic_apply mut clos args dbg =
  match args with
  | [ arg ] ->
    bind "fun" clos (fun clos ->
        Cop (Capply typ_val, [ get_field_gen mut clos 0 dbg; arg; clos ], dbg))
  | _ ->
    let arity = List.length args in
    let cargs = (Cconst_symbol (apply_function_sym arity, dbg) :: args) @ [ clos ] in
...
let send kind met obj args dbg =
  ...
  bind "obj" obj (fun obj ->
      ...
      | Cached, cache :: pos :: args -> call_cached_method obj met cache pos args dbg
  ...
  let clos =
    ...
    Clet
      ...
      , Clet
          ( VP.create cached
          , Cop
              (Cand, [ Cop (Cload (Word_int, Mutable), [ cache ], dbg ()); mask ], dbg ())
          , Clet
              ( VP.create real
              , Cifthenelse
...
let tuplify_function arity =
  ...
  Cfunction
    ...
    ; fun_body =
        Cop
          ( Capply typ_val
          , (get_field_gen Asttypes.Mutable (Cvar clos) 2 (dbg ()) :: access_components 0)
            @ [ Cvar clos ]
          , dbg () )
    ; fun_codegen_options = []
    ; fun_dbg
    ...
    then (
      ...
      Clet
        ...
        , curry_fun
            ...
            (n - 1) ))
  in
  let fun_name = "caml_curry" ^ Int.to_string arity ^ "_" ^ Int.to_string (arity - 1) in
  let fun_dbg = placeholder_fun_dbg ~human_name:fun_name in
  Cfunction
    { fun_name
          ...
          then
            Cop
              ...
              , [ alloc_closure_header 5 (dbg ())
                ...
                ; Cconst_symbol (name1 ^ "_" ^ Int.to_string (num + 1) ^ "_app", dbg ())
                ; Cvar arg
                ; Cvar clos
                ]
              , dbg () )
    ...
    then (
      ...
      let cf =
        Cfunction
          ...
          ; fun_args
          ; fun_body =
              iter (num + 1) (List.map (fun (arg, _) -> Cvar arg) direct_args) clos
          ; fun_codegen_options = []
          ; fun_dbg
          }
      in
      cf :: intermediate_curry_functions arity (num + 1))
    else intermediate_curry_functions arity (num + 1)))
;;
  ...
  return_unit
    ...
    (bind "ref" arg (fun arg ->
         Cop
           ...
           , [ arg
             ; add_const (Cop (Cload (Word_int, Mutable), [ arg ], dbg)) (n lsl 1) dbg
             ]
           , dbg )))
;;
...
let bbswap bi arg dbg =
  ...
  in
  Cop
    ( Cextcall (Printf.sprintf "caml_%s_direct_bswap" prim, typ_int, false, None)
    , [ arg ]
    , dbg )
;;

let bswap16 arg dbg =
  Cop (Cextcall ("caml_bswap16_direct", typ_int, false, None), [ arg ], dbg)
;;
...
let xor_int_caml arg1 arg2 dbg =
  Cop
    ( Cor
    , [ Cop (Cxor, [ ignore_low_bit_int arg1; ignore_low_bit_int arg2 ], dbg)
      ; Cconst_int (1, dbg)
      ]
    , dbg )
;;
...
let stringref_unsafe arg1 arg2 dbg =
  tag_int
    (Cop (Cload (Byte_unsigned, Mutable), [ add_int arg1 (untag_int arg2 dbg) dbg ], dbg))
    dbg
;;
    ...
    bind "index" arg2 (fun idx ->
        bind "arr" arg1 (fun arr ->
            Csequence
              ( make_checkbound
                  dbg
                  [ addr_array_length_shifted (get_header_without_profinfo arr dbg) dbg
                  ; idx
                  ]
              , addr_array_ref arr idx dbg )))
  | Pintarray ->
    ...
    (bind "str" arg1 (fun str ->
         bind "index" (untag_int arg2 dbg) (fun idx ->
             Csequence
               ...
               , Cop
                   ( Cstore (Byte_unsigned, Assignment)
                   , [ add_int str idx dbg; ignore_high_bit_int (untag_int arg3 dbg) ]
                   , dbg ) ))))
;;
...
let cdefine_symbol (symb, (global : Wrap_cmmgen_state.is_global)) =
  match global with
...
let frame_table namelist =
  ...
  in
  Cdata
    ((Cglobal_symbol "caml_frametable"
...
let spacetime_shapes namelist =
  ...
  in
  Cdata
    ((Cglobal_symbol "caml_spacetime_shapes"
...
let emit_constant_closure ((_, global_symb) as symb) fundecls clos_vars cont =
  let closure_symbol (f : Clambda.ufunction) =
    if Config.flambda then cdefine_symbol (f.label ^ "_closure", global_symb) else []
  in
  ...
  | f1 :: remainder ->
    let rec emit_others pos = function
      ...
      | (f2 : Clambda.ufunction) :: rem ->
        ...
        then
          ...
          @ (Csymbol_address f2.label :: cint_const f2.arity :: emit_others (pos + 3) rem)
        else
          (Cint (infix_header pos) :: closure_symbol f2)
    ...
    then Csymbol_address f1.label :: cint_const f1.arity :: emit_others 3 remainder
    else
...
let preallocate_block cont { Clambda.symbol; exported; tag; fields } =
  let space =
    ...
    List.map
      ...
      fields
  in
  let global = Wrap_cmmgen_state.(if exported then Global else Local) in
