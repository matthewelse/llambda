...
val bind_nonvar : string -> expression -> (expression -> expression) -> expression
...
(** Make a Wrap_cmm constant holding the given nativeint value.
...
val div_int : expression -> expression -> Lambda.is_safe -> Debuginfo.t -> expression
val mod_int : expression -> expression -> Lambda.is_safe -> Debuginfo.t -> expression
...
val unboxed_float_array_ref : expression -> expression -> Debuginfo.t -> expression
val float_array_ref : expression -> expression -> Debuginfo.t -> expression
val addr_array_set : expression -> expression -> expression -> Debuginfo.t -> expression
...
val int_array_set : expression -> expression -> expression -> Debuginfo.t -> expression
val float_array_set : expression -> expression -> expression -> Debuginfo.t -> expression
...
val box_int_gen : Debuginfo.t -> Primitive.boxed_integer -> expression -> expression

(** Unbox a given boxed integer *)
val unbox_int : Debuginfo.t -> Primitive.boxed_integer -> expression -> expression

(** Used to prepare 32-bit integers on 64-bit platforms for a lsr operation *)
val make_unsigned_int : Primitive.boxed_integer -> expression -> Debuginfo.t -> expression

val unaligned_load_16 : expression -> expression -> Debuginfo.t -> expression
val unaligned_set_16 : expression -> expression -> expression -> Debuginfo.t -> expression
val unaligned_load_32 : expression -> expression -> Debuginfo.t -> expression
val unaligned_set_32 : expression -> expression -> expression -> Debuginfo.t -> expression
val unaligned_load_64 : expression -> expression -> Debuginfo.t -> expression
val unaligned_set_64 : expression -> expression -> expression -> Debuginfo.t -> expression
...
val simplif_primitive : Clambda_primitives.primitive -> Clambda_primitives.primitive
...
(** Byte swap primitive
    Operates on Wrap_cmm integers (unboxed values) *)
val bbswap : Primitive.boxed_integer -> unary_primitive

(** 16-bit byte swap primitive
    Operates on Wrap_cmm integers (untagged integers) *)
...
(** Generic application of a function to one or several arguments.
    ...
    from the closure. The Wrap_cmmgen code uses a mutable load by
...
(** Generic Wrap_cmm fragments *)

(** Generate generic functions *)
val generic_functions : bool -> Cmx_format.unit_infos list -> Wrap_cmm.phrase list
...
val cdefine_symbol : string * Wrap_cmmgen_state.is_global -> data_item list
...
val emit_block
  :  string * Wrap_cmmgen_state.is_global
  -> nativeint
  -> data_item list
  -> data_item list
