...
let%expect_test "try/with" =
  ...
  [%expect
    ...
    body:                                             ; preds = %entry
      %2 = call ocamlcc i8* @camlTest__g_20(i8* %n_108)
      br label %merge
    ...
    handler1:                                         ; preds = %entry
      ...
      %icmp = icmp eq i64 %4, ptrtoint (i8* @camlTest__Pmakeblock_70 to i64)
      %zext = zext i1 %icmp to i64
      %5 = trunc i64 %zext to i1
